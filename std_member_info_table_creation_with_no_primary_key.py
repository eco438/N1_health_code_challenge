...
def getDistinctMemberCount(cursor):
    return cursor.execute("SELECT COUNT(member_id) from (select DISTINCT member_id from std_member_info)").fetchone()[0]

def getDuplicateMemberCount(cursor):
    return cursor.execute("SELECT COUNT(member_id) FROM (SELECT * FROM std_member_info GROUP BY member_id HAVING COUNT(member_id) > 1)").fetchone()[0]

def getMemberBreakdownByPayer(cursor):
    return cursor.execute("SELECT payer, COUNT(member_id) FROM (select DISTINCT member_id, payer from std_member_info) GROUP BY payer").fetchall()

def getMembersWithAFoodAccessScoreLowerThanTwoCount(cursor):
    return cursor.execute("SELECT COUNT(member_id) from (SELECT member_id, food_access_score from std_member_info left join model_scores_by_zip on zip_code = zcta Where food_access_score < 2 group by member_id ) ").fetchone()[0]

def getAverageSocialIsolationScore(cursor):
    return cursor.execute("SELECT AVG(social_isolation_score) from (SELECT  DISTINCT member_id, social_isolation_score as social_isolation_score from std_member_info left join model_scores_by_zip on zip_code =zcta  )  ").fetchone()[0]

def getMembersWithMaxAlogrexSdohCompositeScore(cursor):
    return cursor.execute(" SELECT DISTINCT member_id,zip_code, algorex_sdoh_composite_score as composite_score from std_member_info left join interview.model_scores_by_zip on zip_code = zcta where algorex_sdoh_composite_score = (SELECT MAX( algorex_sdoh_composite_score) FROM model_scores_by_zip)").fetchall()       
