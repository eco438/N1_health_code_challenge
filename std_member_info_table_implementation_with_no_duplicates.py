...
class StdMemberInfoTableCreate:
    ...
    def insertData(self,table,tableCursor):
        sqlQuery = "SELECT person_id, first_name, last_name, dob, street_address,city,state,zip,payer from {tableName} where ( DATE(eligibility_end_date) is not null and  DATE(eligibility_start_date) is not null and   DATE(eligibility_end_date)  >= '2022-04-01' and  DATE(eligibility_start_date) < '2022-05-01') or ( DATE(substr(eligibility_end_date, 7, 4) || '-' || substr(eligibility_end_date, 1, 2) || '-' || substr(eligibility_end_date, 4, 2)) is not null and  DATE(substr(eligibility_start_date, 7, 4) || '-' || substr(eligibility_start_date, 1, 2) || '-' || substr(eligibility_start_date, 4, 2)) is not null and  DATE(substr(eligibility_end_date, 7, 4) || '-' || substr(eligibility_end_date, 1, 2) || '-' || substr(eligibility_end_date, 4, 2))  >= '2022-04-01' and DATE(substr(eligibility_start_date, 7, 4) || '-' || substr(eligibility_start_date, 1, 2) || '-' || substr(eligibility_start_date, 4, 2))  <  '2022-05-01')".format(tableName=table)
...
def getDistinctMemberCount(cursor):
    return cursor.execute("SELECT COUNT(*) from std_member_info").fetchone()[0]

def getMemberBreakdownByPayer(cursor):
    return cursor.execute("SELECT payer, COUNT(member_id) from std_member_info GROUP BY payer").fetchall()

def getMembersWithAFoodAccessScoreLowerThanTwoCount(cursor):
    return cursor.execute("select count(*) from (SELECT * from std_member_info left join model_scores_by_zip on zip_code = zcta Where food_access_score < 2)").fetchone()[0]

def getAverageSocialIsolationScore(cursor):
    return cursor.execute("SELECT AVG(social_isolation_score) from (SELECT member_id, social_isolation_score as social_isolation_score from std_member_info left join model_scores_by_zip on zip_code =zcta  )  ").fetchone()[0]

def getMembersWithMaxAlogrexSdohCompositeScore(cursor):
    return cursor.execute(" SELECT  member_id,zip_code, algorex_sdoh_composite_score as composite_score from std_member_info left join interview.model_scores_by_zip on zip_code = zcta where algorex_sdoh_composite_score = (SELECT MAX( algorex_sdoh_composite_score) FROM model_scores_by_zip)").fetchall()       


def getStdMemberInfoTableAnalysis(cursor):
    print("1. The number of unique members that are eligible in April 2022 is {}.".format(getDistinctMemberCount(cursor)))
    print("2. The breakdown of members by payer: ")
    for row in getMemberBreakdownByPayer(cursor):
        print("\t\tPayer {}: {}.".format(row[0],row[1]))
    print("3. The number of members thst live in a zip code with a food_access_score lower than 2 is {}.".format(getMembersWithAFoodAccessScoreLowerThanTwoCount(cursor)))
    print("4. The average social isolation score is {}.".format(getAverageSocialIsolationScore(cursor)))
    membersWithMaxAlgorexScoreRowData = getMembersWithMaxAlogrexSdohCompositeScore(cursor)
    print("5. The highest algorex_sdoh_composite_score is {} in zip code {}. The members with the highest algorex_sdoh_composite_score are: ".format(membersWithMaxAlgorexScoreRowData[0][-1],membersWithMaxAlgorexScoreRowData[0][-2]))
    for row in membersWithMaxAlgorexScoreRowData:
        memberInformation = "\t\tMemberId: {}".format(*row)
